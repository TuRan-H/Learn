...
class GroupQueryAttention(nn.Module):
	...
	def forward(self, X: torch.Tensor, attention_mask = None):
		...
		Q = Q.view(batch_size, seq_length, self.nums_head, self.head_dim).transpose(1, 2)		# (batch_size, nums_head, seq_length, head_dim)
		K = K.view(batch_size, seq_length, self.nums_kv_head, self.head_dim).transpose(1, 2)	# (batch_size, nums_kv_head, seq_length, head_dim)
		...
		attention_score = Q @ V.transpose(-1, -2) / math.sqrt(self.head_dim)		# (batch_size, nums_head, seq_length, seq_length)
		...
		output = output.transpose(1, 2).contiguous().view(batch_size, seq_length, -1)		# (batch_size, seq_length, nums_head * head_dim)
